...
public class Chess {
    ...
    public static ReturnPlay play(String moveStr) {
        ...
        result.piecesOnBoard = board.getPieces();
        if(board.gameOver){
            if(currentPlayer==Player.WHITE) result.message = ReturnPlay.Message.CHECKMATE_WHITE_WINS;
            else result.message = ReturnPlay.Message.CHECKMATE_BLACK_WINS;
            return result;
        }
        if (!success) {
        ...
        boolean opponentIsWhite = (currentPlayer != Player.WHITE); // BLACK moved? opponent is WHITE; WHITE moved? opponent is BLACK
